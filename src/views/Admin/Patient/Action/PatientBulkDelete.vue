<template>
    <Modal :is-open="props.onOpen" @close="props.onClose" class="max-w-md m-4">
        <div class="relative w-full max-w-md rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
            ...
            <div class="text-center">
                ...
                <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                    ...
                    <br />
                    <b>{{ props.ids.length }}</b> data pasien terpilih?
                </p>

                <div class="flex items-center gap-3 px-2 mt-6 lg:justify-end">
                    <Button size="sm" class="w-full" :disabled="isDeleting" @click="props.onClose">
                        Tidak
                    </Button>
                    <Button size="sm" variant="outline" class="w-full" :disabled="isDeleting" @click="onDelete">
...
const { deleteData } = useDeletePatient();
const isDeleting = ref(false);

const onDelete = async () => {
    isDeleting.value = true;

    const results = await Promise.all(props.ids.map((id) => deleteData(id)));
    const failedCount = results.filter((result) => result.error).length;

    isDeleting.value = false;

    if (failedCount > 0) {
        toast.error(`Gagal menghapus ${failedCount} dari ${props.ids.length} data pasien`);
    } else {
        toast.success(`Berhasil menghapus ${props.ids.length} data pasien!`);
    }
