        ...
        <div class="relative w-full max-w-md rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
            ...
            <div class="text-center">
                ...
                <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                    ...
                    <br />
                    <b class="uppercase">{{ props.name }}</b>?
                </p>

                <div class="flex items-center gap-3 px-2 mt-6 lg:justify-end">
                    <Button size="sm" class="w-full" @click="props.onClose">
                        Tidak
                    </Button>
                    <Button size="sm" variant="outline" class="w-full" @click="onDelete">
                        Ya
                    </Button>
                </div>
            </div>
...
const onDelete = async () => {
    const { error, response } = await deleteData(props.id);
    if (error || response) {
        if (error) {
