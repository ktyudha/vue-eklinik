    ...
    <div class="space-y-1">
        <div>
            <label v-if="label" class="text-theme-sm font-normal text-gray-500 dark:text-gray-400">
                ...
                <small v-show="required" class="text-red-500">*</small>
            </label>

            <div class="relative">
                <input :type="inputType" :placeholder="placeholder" v-model="value" :required="required"
                    @blur="handleBlur" @input="emit('update:modelValue', ($event.target as HTMLInputElement).value)"
                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-4 pr-11 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />

                <span v-show="isPassword" @click="togglePasswordVisibility"
